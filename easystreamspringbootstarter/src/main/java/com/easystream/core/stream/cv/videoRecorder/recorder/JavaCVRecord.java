...
public class JavaCVRecord implements Recorder {
	 ...
	 * @throws Exception
	 */
	public Recorder from(String src) throws CodecNotFoundExpception, Exception {
		...
		grabber.start("5120");// 开始之后ffmpeg会采集视频信息，之后就可以获取音视频信息
		width = grabber.getImageWidth();
		height = grabber.getImageHeight();
		if (width == 0 && height == 0) {
			System.out.println(src + " tttttttttttttttttttttt 拉流异常！");
			grabber.stop();
			grabber.close();
			return null;
		}
	...
	public Recorder to(String out) throws IOException {
		...
		this.out = out;
		System.out.println(out+"width "+width+"  height "+height);
		// 录制/推流器
	...
	public JavaCVRecord start() {
		if (cuThread == null) {
			...
			stop();
			cuThread.reset(grabber, record,this);// 重置
			cuThread.go();
		}
	 ...
	 **/
	 @Override
	public Recorder resRecord(String out) throws IOException {
		 return to(out).start();
	}
